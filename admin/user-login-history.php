...
if (!empty($search)) {
    ...
    $whereClause = "WHERE 
        LOGS_FNAME LIKE '%$searchTerm%' OR
        LOGS_LNAME LIKE '%$searchTerm%' OR
        LOGS_USER_ROLE LIKE '%$searchTerm%' OR
        LOGS_ACTIVITY LIKE '%$searchTerm%' OR
        LOGS_DATE LIKE '%$searchTerm%'";
}
...
$totalQuery = "SELECT COUNT(*) AS total FROM user_logs $whereClause";
...
$sql = "SELECT * FROM user_logs $whereClause ORDER BY LOGS_ID DESC LIMIT $limit OFFSET $offset";
...
<body>
    <div class="content w-[calc(100vw-60px)] mx-auto overflow-x-auto p-8 px-8">
        <div class="bg-white shadow-md rounded-lg p-6 mb-6">
            <h2 class="font-bold mb-4 text-[13px] xs:text-[12px] sm:text-[15px] font-bold text-[#0047bb]">User Login
                History</h2>
                    ...
                    <?php if ($result->num_rows > 0): ?>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr class="hover:bg-gray-200 border-b border-gray-300">
                                <td class="px-4 py-2">
                                    <?= htmlspecialchars($row['LOGS_FNAME'] . " " . htmlspecialchars($row['LOGS_LNAME'])) ?>
                                </td>
                                <td class="px-4 py-2"><?= htmlspecialchars($row['LOGS_USER_ROLE']) ?></td>
                                <td class="px-4 py-2"><?= htmlspecialchars($row['LOGS_ACTIVITY']) ?></td>
                                <td class="px-4 py-2">
                                    <?= htmlspecialchars(date('M j, Y \a\t g:i a', strtotime($row['LOGS_DATE']))) ?>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
