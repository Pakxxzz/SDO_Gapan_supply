...
$whereClause = "WHERE OFF_IS_ARCHIVED = 0";
...
$totalQuery = "SELECT COUNT(*) AS total FROM office $whereClause";
...
<body>
    <div class="content w-[calc(100vw-60px)] mx-auto overflow-x-auto p-8 px-8">
        <div class="bg-white shadow-md rounded-lg p-6 mb-6">
            <h2 class="font-bold mb-4 text-[13px] xs:text-[12px] sm:text-[15px] font-bold text-[#0047bb]">Office List
            </h2>
                        ...
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr class="hover:bg-gray-200 border-b border-gray-300">
                                ...
                                <td class="action-column flex justify-left items-center space-x-2 px-4 py-2">
                                    ...
                                    <button onclick="deleteItem(<?= $row['OFF_ID'] ?>)" class="text-red-600 hover:text-red-800">
    ...
    <div id="modal" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-[1000] hidden">
        <div class="bg-white m-3 rounded-lg shadow-lg p-6 w-96 max-h-[90vh] overflow-y-auto">
            <h2 id="modalText" class="text-xl font-bold mb-4">Add New Office</h2>
            <form id="form" method="POST">
                <input type="hidden" name="offId" id="offId">
                <div class="mb-3">
                    <label class="block font-medium" for="offCode">Office Code</label>
                    <input type="text" id="offCode" name="offCode" class="w-full border rounded px-3 py-2" required>
                </div>
                <div class="mb-3">
                    <label class="block font-medium" for="offName">Office Name</label>
                    <input type="text" id="offName" name="offName" class="w-full border rounded px-3 py-2" required>
                </div>
...
<script src="office.js?id=<?= time() ?>"></script>
